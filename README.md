...
The inventory covers **638 Maths and Science PDF or ZIP-member sources (13,954 pages)** in the parent `../Data` folder. Text extraction has staged **13,883 pages** for audit; the remaining 71 pages belong to two malformed 2025 model-paper PDFs. Staging is source accounting, not approval for student answers. The deployment bundles 299 individually reviewed addendum chunks alongside the 49-record syllabus/NCERT pilot, for 348 unique records after owner publication. Most practice papers and textbook pages still need visual, syllabus, and question/marking-scheme checks. The owner-only publishing endpoint accepts only reviewed records bundled into the deployment. `../Data/reports/all-maths-science-source-coverage.json` records the exact status of each source.
