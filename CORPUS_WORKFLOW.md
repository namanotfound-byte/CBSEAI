...
3. A marking scheme can supply marks only when it is joined to its exact paper, question and part. Five diagram-free questions from the current Science sample paper now have page-checked, exact marking-scheme joins. The remaining paper questions are still pending review.
...
| `../Data/processed/science-maths-2026-27/maths-reviewed-addendum.jsonl` | Four page-checked Maths passages with source review notes | Approved and indexed in the 59-row live pilot |
| `../Data/processed/science-maths-2026-27/science-reviewed-addendum.jsonl` | Six page-checked Science passages with source review notes | Approved and indexed in the 59-row live pilot |
| `../Data/processed/science-maths-2026-27/science-concept-reviewed-addendum.jsonl` | Thirteen visually checked Science concept passages | Approved and indexed in the 94-row live pilot |
| `../Data/processed/science-maths-2026-27/maths-reviewed-addendum-2.jsonl` | Twelve visually checked Maths passages with verified formulas | Approved and indexed in the 94-row live pilot |
| `../Data/processed/science-maths-2026-27/science-sqp-reviewed-addendum.jsonl` | Five current Science questions and five matching marking-scheme rows | Approved and indexed in the 94-row live pilot |
...
No model is installed locally. Hosted answers now try Groq Free Qwen3.8 27B, then free OpenRouter routes; BGE-M3 embeddings use Cloudflare Workers AI's daily free allowance via API. Sparse retrieval uses deterministic lexical weights with Qdrant. The 94-point pilot collection `cbse_10_pilot_20260925_v3` backs the live Padhle domain. A live “What is a lens?” check returned the NCERT definition with the Chapter 9 citation; a “What is a prism?” check produced an in-syllabus source-gap message. After the first ten-row addendum, a live “What is the SSS similarity criterion?” check returned the reviewed Chapter 6 Maths passage and citation. A further 35 page-checked records were published idempotently by the owner-only `/admin/corpus` route on 26 September 2026, and the live count was verified as 94. The five current Science sample-paper questions each carry an exact marking-scheme partner; they are not representative of complete practice-paper coverage. Future large batches should use the versioned-collection and alias workflow above. Chapter routing no longer counts a partial match on a multiword alias such as “eye lens”, and answer retrieval requires question-to-passage overlap so neighbouring paragraphs cannot be used as evidence. The earlier 47-point and 39-point pilot collections remain separate. Keep API keys in protected Vercel environment variables, never in this repository or a chat message. Free answer models can be rate-limited; the tutor shows a labelled NCERT excerpt when a model is busy and suitable evidence exists.
