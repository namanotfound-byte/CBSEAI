...
No model is installed locally. Hosted answers now try Groq Free Qwen3.8 27B, then free OpenRouter routes; BGE-M3 embeddings use Cloudflare Workers AI's daily free allowance via API. Sparse retrieval uses deterministic lexical weights with Qdrant. The 59-point pilot collection `cbse_10_pilot_20260925_v3` backs the live Padhle domain. A live “What is a lens?” check returned the NCERT definition with the Chapter 9 citation; a “What is a prism?” check produced an in-syllabus source-gap message. After the ten-row addendum, a live “What is the SSS similarity criterion?” check returned the reviewed Chapter 6 Maths passage and citation. These ten fixed reviewed records were published idempotently by the owner-only `/admin/corpus` route; future large batches should use the versioned-collection and alias workflow above. Chapter routing no longer counts a partial match on a multiword alias such as “eye lens”, and answer retrieval requires question-to-passage overlap so neighbouring paragraphs cannot be used as evidence. The earlier 47-point and 39-point pilot collections remain separate. Keep API keys in protected Vercel environment variables, never in this repository or a chat message. Free answer models can be rate-limited; the tutor shows a labelled NCERT excerpt when a model is busy and suitable evidence exists.
