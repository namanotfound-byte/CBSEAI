...
| `../Data/processed/science-maths-2026-27/optics-reviewed-addendum.jsonl` | Visually checked NCERT Table 9.4 row for a convex-lens image enlarged on a screen | One approved row; passes local retrieval, awaiting staging ingestion |
...
No model is installed locally. Hosted answers now try Groq Free Qwen3.8 27B, then free OpenRouter routes; BGE-M3 embeddings use Cloudflare Workers AI's daily free allowance via API. Sparse retrieval uses deterministic lexical weights with Qdrant. The six production credentials passed isolated checks, including a Groq-only cited answer test. The 47-point pilot collection `cbse_10_pilot_20260924_v2` is separate from the missing live alias. Its retrieval checks passed for three Science questions, two Maths questions and two deliberate missing-evidence cases. The earlier 39-point pilot collection remains unaliased. Keep API keys in protected Vercel environment variables, never in this repository or a chat message. The OpenRouter free routes were upstream rate-limited during generation testing.
